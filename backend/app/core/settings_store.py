...
FIELDS: List[Field] = [
    ...
    Field("SHAREPOINT_SYNC_ROOTS", "SharePoint / OneDrive sync roots", "documents",
          ...
          show_if=("DOCUMENTS_ENABLED", [True])),
    Field("SHAREPOINT_GRAPH_ENABLED", "Connect directly to SharePoint", "documents",
          "Optional. Uses Microsoft Graph to search and read remote-only files. "
          "Your administrator must provide a public-client application ID.",
          type="bool", show_if=("DOCUMENTS_ENABLED", [True])),
    Field("MICROSOFT_TENANT_ID", "Microsoft tenant", "documents",
          "Tenant domain or ID. Use 'common' for a multi-tenant app.",
          placeholder="contoso.onmicrosoft.com",
          show_if=("SHAREPOINT_GRAPH_ENABLED", [True])),
    Field("MICROSOFT_CLIENT_ID", "Microsoft application ID", "documents",
          "Public-client application ID approved by your Microsoft 365 administrator. "
          "Cerebro uses delegated read-only permissions.",
